...
namespace WorkerRole
{
    public class InputJobServer
    {
        ...
        public InputJobServer()
        {
            ...
            try
            {
                serviceHost.AddServiceEndpoint(typeof(IBrotherConnection), binding, endpointAddress);
        ...
        public void Open()
        {
            ...
            {
                serviceHost.Open();
                Trace.TraceInformation(String.Format("Host for {0} endpoint type opened successfully at {1} {2}", externalEndpointName, DateTime.Now,endpointAddress));
            }
            catch (Exception e)
            {
